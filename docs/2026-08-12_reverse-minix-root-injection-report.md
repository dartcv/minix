...
> **历史命名提示（2026-08-18）：** 文件名中的 `root-injection` 和正文中的 `Root*` 标识只保留时间线与源码兼容性。当前实现是固定证书 + `android:sharedUserId="ca.sailboat.a"` + 同 Linux UID + 普通非导出 `:control` Service/Binder；运行链路未启用 Root shell、`su`、UID 0、Magisk、Shizuku 或独立特权守护进程。详见 [`2026-08-18_架构-Root方案弃用说明-report.md`](2026-08-18_架构-Root方案弃用说明-report.md)。
