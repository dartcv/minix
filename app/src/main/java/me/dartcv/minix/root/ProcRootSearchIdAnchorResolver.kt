...
internal class ProcRootSearchIdAnchorResolver(
    ...
    override fun resolve(pid: Int, profile: RootSearchIdProfile): RootSearchIdAnchorResult {
        val result = runCatching { resolveChecked(pid, profile) }
            .getOrElse {
                failure(
                    RootSearchIdAnchorStatus.UNAVAILABLE,
    ...
    private fun resolveChecked(
        ...
        if (pid <= 0) {
            return failure(RootSearchIdAnchorStatus.INVALID_PID, "PID must be positive")
        }
        ...
        if (
            ...
            expectedSha256 == null ||
            profile.anchorKind != RootModuleAnchorKind.MODULE_SPEC_RESULT
        ) {
            return failure(
                RootSearchIdAnchorStatus.INVALID_PROFILE,
        ...
        val startTimeBefore = readStartTimeTicks(pid)
            ...
            ?: return failure(
                RootSearchIdAnchorStatus.TARGET_CHANGED,
        ...
        if (moduleRows.isEmpty()) {
            return failure(
                RootSearchIdAnchorStatus.MODULE_NOT_FOUND,
        ...
        if (initialLoadRows.isEmpty()) {
            return failure(
                RootSearchIdAnchorStatus.ANCHOR_NOT_FOUND,
        ...
        if (matchedPaths.size != 1) {
            return failure(
                RootSearchIdAnchorStatus.MODULE_AMBIGUOUS,
        ...
        val initialSelection = selectAnchor(
            ...
            requiredRootOffset = profile.recipe.rootOffset,
        )
    ...
    private fun selectAnchor(
        ...
        if (selectedLoads.isEmpty()) {
            return AnchorSelectionResult(
                failureStatus = RootSearchIdAnchorStatus.ANCHOR_NOT_FOUND,
                message = "Fingerprint-matched module lost its offset-zero mapping",
            )
        }
        if (selectedLoads.size != 1) {
            return AnchorSelectionResult(
                failureStatus = RootSearchIdAnchorStatus.MODULE_AMBIGUOUS,
                message = "Fingerprint-matched module has multiple offset-zero mappings",
            )
        }
        val loadRow = selectedLoads.single()
        if (!hasSuffixToken) {
            if (!loadRow.covers(loadRow.start, requiredRootOffset, INT32_BYTES)) {
                return AnchorSelectionResult(
                    failureStatus = RootSearchIdAnchorStatus.ANCHOR_NOT_FOUND,
                    message = "Offset-zero module mapping does not cover the required root seed",
                )
            }
            return AnchorSelectionResult(
        ...
        val expectedLogicalEnd = checkedAdd(
            loadRow.start,
            RootGameAppArtifact1582.LOAD_VIRTUAL_END,
        ) ?: return AnchorSelectionResult(
            failureStatus = RootSearchIdAnchorStatus.ANCHOR_NOT_FOUND,
        ...
        if (
            ...
            !anchorRow.covers(anchorRow.start, requiredRootOffset, INT32_BYTES)
        ) {
            return AnchorSelectionResult(
                failureStatus = RootSearchIdAnchorStatus.ANCHOR_NOT_FOUND,
    ...
    private fun readMaps(pid: Int): MapsSnapshot {
        val mapsFile = File(procRoot, "$pid/maps")
        return runCatching {
            var lineCount = 0
            var byteCount = 0L
    ...
    private fun failure(
        status: RootSearchIdAnchorStatus,
        message: String,
    ...
    private data class AnchorSelectionResult(
        ...
        val failureStatus: RootSearchIdAnchorStatus? = null,
        val message: String = "",
    )
