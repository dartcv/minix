...
internal class ProcTargetProcessInspector(
    ...
    override fun findPid(packageName: String): Int? {
        if (!RootProtocol.isValidPackageName(packageName)) return null
        ...
        procRoot.listFiles()
            ...
            .forEach { pid ->
                ...
                if (fallbackPid == null && RootProtocol.processMatchesPackage(processName, packageName)) {
                    fallbackPid = pid
                }
            }
        return fallbackPid
    ...
    override fun readProcessName(pid: Int): String? {
        ...
        return runCatching {
            File(procRoot, "$pid/cmdline").inputStream().use { input ->
    ...
    override fun readStartTimeTicks(pid: Int): String? {
        if (pid <= 0) return null
        return runCatching {
            val statLine = File(procRoot, "$pid/stat").bufferedReader().use { reader ->
                reader.readLine()
    ...
    override fun readEffectiveUid(pid: Int): Int? {
        if (pid <= 0) return null
        return runCatching {
            val uidRows = File(procRoot, "$pid/status").bufferedReader().useLines { lines ->
                lines
    ...
    override fun readBrief(pid: Int): TargetProcessBrief? {
        ...
        val mapCount = runCatching {
            File(procRoot, "$pid/maps").bufferedReader().useLines { lines ->
    ...
    override fun isAlive(pid: Int): Boolean =
        pid > 0 && File(procRoot, pid.toString()).isDirectory && readProcessName(pid) != null

    private fun readStatusValues(pid: Int): Map<String, String> {
        return runCatching {
            val values = mutableMapOf<String, String>()
            File(procRoot, "$pid/status").bufferedReader().useLines { lines ->
...
internal class RootTargetSession(
    ...
    private val fieldProfileResolver: RootReadOnlyFieldProfileResolver =
        RootReadOnlyFieldProfileCatalog.resolver,
    private val addressRecipeResolver: RootAddressRecipeResolver = NoRootAddressRecipeResolver,
    private val moduleSpecFieldAddressResolver: RootModuleSpecFieldAddressResolver =
        NoRootModuleSpecFieldAddressResolver,
    private val searchIdScanner: RootSearchIdScanner? = null,
    private val searchIdProfile: RootSearchIdProfile =
        RootRecoveredSearchIdProfiles.miniWorld1582Draft,
    private val serviceAbi: String = "",
    private val injectionProfileResolver: RootInjectionProfileResolver =
        RootInjectionProfileCatalog.resolver,
    private val injectionExecutor: RootInjectionExecutor = NoRootInjectionExecutor,
    ...
    private val featureStates = defaultRootFeatureStates().toMutableMap()
    private var nativeProbeState = RootNativeProbeState()
    private var nativeModules: List<RootNativeModuleIdentity> = emptyList()
    private var fieldProfileResolution = RootReadOnlyFieldProfileResolution()
    private var injectionProfileResolution = RootInjectionProfileResolution()

    private data class PreparedPositionAxis(
        val axis: RootPlayerPositionAxis,
        val address: Long,
    ...
    fun findTargetPid(value: String): Int? {
        ...
        if (!RootProtocol.isValidPackageName(normalized)) return null
    ...
    fun verifyTargetProcess(value: Int, targetPackage: String): Boolean {
        val normalized = RootProtocol.normalizePackageName(targetPackage)
    ...
    fun openOrRefresh(value: String): Boolean {
        val normalized = RootProtocol.normalizePackageName(value)
        if (!RootProtocol.isValidPackageName(normalized)) {
            close("目标包名格式无效")
            return false
        }
        ...
        if (
            ...
            !RootProtocol.processMatchesPackage(processName, normalized)
        ) {
    ...
    @Synchronized
    fun setFeatureEnabled(feature: RootFeature, enabled: Boolean): Boolean {
        ...
        if (feature == RootFeature.PLAYER_TELEPORT) {
            recordInjectionApply(
                RootInjectionApplyStatus.FEATURE_NOT_DEFINED,
                feature,
                "Player teleport is an explicit three-axis action, not a boolean feature",
            )
            return false
        }
        if (feature != RootFeature.READABLE_DATA) {
    ...
    @Synchronized
    fun setPlayerPosition(request: RootPlayerPositionRequest): RootPlayerPositionResult {
        val activePid = pid ?: return positionFailure(
            request,
            RootInjectionApplyStatus.SESSION_CLOSED,
            "Target session is not open",
        )
        val verification = inspectTargetIdentity(activePid, packageName)
        if (!verification.isValid) {
            val message = verification.message
            invalidateTargetForInjection(RootFeature.PLAYER_TELEPORT, message)
            return RootPlayerPositionResult(
                request = request,
        ...
        if (!resolution.state.isProfileReady || profile == null) {
            return positionFailure(
                request,
                RootInjectionApplyStatus.PROFILE_NOT_READY,
        ...
        val requestedWorldValues = mapOf(
            RootPlayerPositionAxis.X to request.x,
            RootPlayerPositionAxis.Y to request.y,
            RootPlayerPositionAxis.Z to request.z,
        )
        val requestedValues = linkedMapOf<RootPlayerPositionAxis, Long>()
        for (axis in RootPlayerPositionAxis.entries) {
        ...
        for (axis in RootPlayerPositionAxis.entries) {
            ...
            val addressResult = runCatching {
                ...
                return positionFailure(
                    request,
                    RootInjectionApplyStatus.PRECONDITION_READ_FAILED,
        ...
        for (axisWrite in pending) {
            val writeResult = runCatching {
                injectionExecutor.apply(
                    RootInjectionWriteRequest(
                        pid = activePid,
                        expectedStartTimeTicks = pinnedStartTime,
                        profileId = profile.profileId,
                        feature = RootFeature.PLAYER_TELEPORT,
            ...
            if (writeResult.status == RootInjectionApplyStatus.APPLIED) {
                written += axisWrite
            }
            if (writeResult.status == RootInjectionApplyStatus.TARGET_CHANGED ||
            ...
            if (!writeResult.isSuccess || writeResult.processStartTimeTicks.isBlank()) {
                ...
                val status = if (rollbackOk) {
                    ...
                    RootInjectionApplyStatus.ROLLBACK_FAILED
                }
                return positionFailure(
                    request,
                    status,
            ...
            if (!verificationMatches) {
                ...
                return positionFailure(
                    request,
                    if (rollbackOk) {
                        RootInjectionApplyStatus.VERIFY_FAILED
                    } else {
                        RootInjectionApplyStatus.ROLLBACK_FAILED
                    },
        ...
        recordInjectionApply(status, RootFeature.PLAYER_TELEPORT, message)
        summary = buildSummary(inspector.readProcessName(activePid).orEmpty())
        return RootPlayerPositionResult(
            request = request,
            effectiveX = request.x,
    ...
    @Synchronized
    fun isFeatureEnabled(feature: RootFeature): Boolean = featureStates[feature] == true

    @Synchronized
    fun enabledFeatureIds(): Array<String> = featureStates
        .filterValues { it }
        .keys
        .map(RootFeature::wireId)
        .toTypedArray()
    ...
    @Synchronized
    fun fieldProfileState(): RootReadOnlyFieldProfileState = fieldProfileResolution.state

    @Synchronized
    fun injectionState(): RootInjectionState = injectionProfileResolution.state

    @Synchronized
    fun antiFlashStartRequest(): RootAntiFlashStartRequest? {
        ...
        return RootAntiFlashStartRequest(
            pid = activePid,
     ...
     */
    @Synchronized
    fun headSpinStartRequest(intervalInput: Int): RootHeadSpinStartRequest? {
        val activePid = pid ?: return null
        val activeStartTime = startTimeTicks?.takeIf(String::isNotBlank) ?: return null
        val activeMapsGeneration = mapsFingerprint?.takeIf(String::isNotBlank) ?: return null
        if (!nativeProbeState.isMemoryReady ||
        ...
        return RootHeadSpinStartRequest(
            identity = RootHeadSpinTargetIdentity(
                pid = activePid,
                startTimeTicks = activeStartTime,
    ...
    fun isAntiFlashProfileReady(): Boolean {
        ...
        val profile = RootAntiFlashProfileCatalog.profile
    ...
    @Synchronized
    fun readVisibleFields(): RootReadOnlyFieldBatch {
        val activePid = pid
        if (activePid != null && !refreshMemoryGeneration(activePid, "visible-field read")) {
            val failure = fieldReadFailure(
                RootReadOnlyFieldReadStatus.TARGET_CHANGED,
                summary,
            )
            return RootReadOnlyFieldBatch(fieldProfileResolution.state, failure, failure, failure)
        }
        val profileState = fieldProfileResolution.state
        if (!profileState.isReady) {
            val failure = fieldReadFailure(
                RootReadOnlyFieldReadStatus.PROFILE_NOT_READY,
                profileState.message.ifBlank { profileState.summary },
            )
            return RootReadOnlyFieldBatch(profileState, failure, failure, failure)
        }
        if (featureStates[RootFeature.READABLE_DATA] != true) {
            val failure = fieldReadFailure(
        ...
        return RootReadOnlyFieldBatch(
            profileState = profileState,
            lifeState = readField(RootReadOnlyFieldId.LIFE_STATE),
            killCount = readField(RootReadOnlyFieldId.KILL_COUNT),
            dataLongSelector1 = readField(RootReadOnlyFieldId.DATA_LONG_SELECTOR_1),
        )
    }

    @Synchronized
    fun readField(fieldId: RootReadOnlyFieldId): RootReadOnlyFieldReadResult {
        val activePid = pid ?: return fieldReadFailure(
            RootReadOnlyFieldReadStatus.SESSION_CLOSED,
        ...
        if (!refreshMemoryGeneration(activePid, "field read")) {
            return fieldReadFailure(RootReadOnlyFieldReadStatus.TARGET_CHANGED, summary)
        }
        if (!fieldProfileResolution.state.isReady) {
            return fieldReadFailure(
                RootReadOnlyFieldReadStatus.PROFILE_NOT_READY,
        ...
        val resolvedField = fieldProfileResolution.profile
            ...
            ?.get(fieldId)
            ?: return fieldReadFailure(
                RootReadOnlyFieldReadStatus.FIELD_NOT_AVAILABLE,
        ...
        val address = resolvedField.address ?: run {
            ...
            if (
                addressResult.status == RootAddressRecipeStatus.TARGET_CHANGED ||
        ...
        val scalarAttempt = runCatching {
            when (resolvedField.definition) {
                is RootInt32FieldDefinition -> scalarReader.readInt32(activePid, address)
                is RootInt64FieldDefinition -> scalarReader.readInt64(activePid, address)
            }
        }
        if (inspector.readStartTimeTicks(activePid) != pinnedStartTime) {
        ...
        val value = when (val definition = resolvedField.definition) {
            is RootInt32FieldDefinition -> RootInt32FieldValue(
                id = definition.id,
                value = definition.decode(valueBits),
            )
            is RootInt64FieldDefinition -> RootInt64FieldValue(
                id = definition.id,
                value = definition.decode(valueBits),
            )
        }
        return RootReadOnlyFieldReadResult(
    ...
    @Synchronized
    fun searchId(requestedId: Long): RootSearchIdResult {
        val activePid = pid ?: return searchIdFailure(
            requestedId,
            RootSearchIdInvalidReason.INVALID_PID,
            "Target session is closed",
        )
        ...
        val scanner = searchIdScanner ?: return searchIdFailure(
            requestedId,
            RootSearchIdInvalidReason.ANCHOR_UNAVAILABLE,
        ...
        if (result.invalidReason == RootSearchIdInvalidReason.TARGET_CHANGED) {
    ...
    @Synchronized
    fun snapshot(): RootTargetSnapshot = RootTargetSnapshot(
    ...
    private fun buildSummary(processName: String): String {
        ...
        val details = buildList {
            ...
            if (nativeProbeState.status != RootNativeProbeStatus.IDLE) {
    ...
    private fun resetFeatures() {
        RootFeature.entries.forEach { featureStates[it] = false }
    }

    private fun supportedFeatures(): Set<RootFeature> = buildSet {
        if (isAntiFlashProfileReady()) {
            add(RootFeature.ANTI_FLASH)
        }
        if (nativeProbeState.isMemoryReady && fieldProfileResolution.state.isReady) {
            add(RootFeature.READABLE_DATA)
        }
        if (nativeProbeState.isMemoryReady && injectionProfileResolution.state.isProfileReady) {
            ...
            if (injectionProfileResolution.profile?.playerPosition != null) {
                add(RootFeature.PLAYER_TELEPORT)
            }
        }
    }
    ...
    private fun rollbackPlayerPosition(
        ...
        profileId: String,
        evidence: RootPlayerPositionEvidence,
        ...
        for (axisWrite in written.asReversed()) {
            ...
            if (rollback == null || !rollback.isSuccess ||
                ...
                allRolledBack = false
                if (rollback?.status == RootInjectionApplyStatus.TARGET_CHANGED) break
                continue
            }
            val verification = readInjectionScalarSafely(
                activePid,
    ...
    private fun disableUnsupportedFeatures() {
        ...
        RootFeature.entries.forEach { feature ->
    ...
    private fun applyInjectionFeature(
        ...
        val writeRequest = RootInjectionWriteRequest(
            pid = activePid,
            expectedStartTimeTicks = pinnedStartTime,
            profileId = profile.profileId,
            feature = feature,
    ...
    private fun invalidateTargetForInjection(
        ...
        injectionProfileResolution = RootInjectionProfileResolution(
    ...
    private fun resetNativeProbe() {
        nativeProbeState = RootNativeProbeState()
        nativeModules = emptyList()
        mapsFingerprint = null
        fieldProfileResolution = RootReadOnlyFieldProfileResolution()
        injectionProfileResolution = RootInjectionProfileResolution()
    }
    ...
    private fun inspectTargetIdentity(value: Int, normalizedPackage: String): TargetVerification {
        if (!RootProtocol.isValidPackageName(normalizedPackage)) {
        ...
        if (!RootProtocol.processMatchesPackage(processName, normalizedPackage)) {
