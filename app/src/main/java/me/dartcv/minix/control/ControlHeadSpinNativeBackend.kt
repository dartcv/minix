...
import me.dartcv.minix.control.nativeadapter.NativeU32BatchResult
 ...
 */
internal object JniControlHeadSpinBackend : ControlHeadSpinBackend {
    override fun readPreflight(target: ControlHeadSpinTarget): ControlHeadSpinPreflightResult {
        val result = runCatching {
            ...
            return ControlHeadSpinPreflightResult(
        ...
        if (code == null || axisA == null || axisB == null) {
            return ControlHeadSpinPreflightResult(
                status = ControlHeadSpinBackendStatus.INVALID_RESPONSE,
                processStartTimeTicks = result.processStartTimeTicks,
                mapsFingerprint = result.mapsFingerprint,
    ...
    private fun guardedBatch(
        ...
        NativeU32BatchResult(
            ...
            operation = me.dartcv.minix.control.nativeadapter.NativeU32BatchOperation.ROLLBACK,
            pid = target.identity.pid,
            processStartTimeTicks = target.identity.startTimeTicks,
            mapsFingerprint = target.identity.mapsFingerprint,
...
private fun NativeMemoryBatchStatus.toHeadSpinBackendStatus(): ControlHeadSpinBackendStatus = when (this) {
    NativeMemoryBatchStatus.OK -> ControlHeadSpinBackendStatus.OK
    NativeMemoryBatchStatus.TARGET_CHANGED,
    NativeMemoryBatchStatus.TARGET_NOT_RUNNING,
    -> ControlHeadSpinBackendStatus.TARGET_CHANGED
    ...
    -> ControlHeadSpinBackendStatus.READ_FAILED
    NativeMemoryBatchStatus.WRITE_FAILED,
    NativeMemoryBatchStatus.PARTIAL_WRITE,
    -> ControlHeadSpinBackendStatus.WRITE_FAILED
    NativeMemoryBatchStatus.VERIFY_FAILED -> ControlHeadSpinBackendStatus.VERIFY_FAILED
    NativeMemoryBatchStatus.NATIVE_UNAVAILABLE,
    NativeMemoryBatchStatus.NATIVE_ERROR,
    NativeMemoryBatchStatus.MEM_OPEN_FAILED,
    -> ControlHeadSpinBackendStatus.UNAVAILABLE
    ...
    -> ControlHeadSpinBackendStatus.INVALID_RESPONSE
}
