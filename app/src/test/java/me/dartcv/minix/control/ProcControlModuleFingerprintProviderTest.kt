...
class ProcControlModuleFingerprintProviderTest {
    @get:Rule
    val temporaryFolder = TemporaryFolder()

    @Test
    fun deletedMappingUsesExactOffsetZeroLoadBaseMapFile() {
        val procControl = temporaryFolder.newFolder("proc-deleted")
        val process = File(procControl, PID.toString()).apply { mkdirs() }
        ...
        assertEquals(
            ...
            ProcControlModuleFingerprintProvider.sha256(PID, DELETED_PATH, 0x1000, procControl),
        )
    }

    @Test
    fun deletedMappingWithoutExactUniqueLoadBaseFailsClosed() {
        val procControl = temporaryFolder.newFolder("proc-deleted-fail")
        val process = File(procControl, PID.toString()).apply { mkdirs() }
        val mapFiles = File(process, "map_files").apply { mkdirs() }
        ...
        assertNull(ProcControlModuleFingerprintProvider.sha256(PID, DELETED_PATH, 0x3000, procControl))
        assertNull(ProcControlModuleFingerprintProvider.sha256(PID, DELETED_PATH, 0, procControl))
    }

    @Test
    fun deletedMappingRejectsNonZeroFileOffset() {
        val procControl = temporaryFolder.newFolder("proc-deleted-offset")
        val process = File(procControl, PID.toString()).apply { mkdirs() }
        val mapFiles = File(process, "map_files").apply { mkdirs() }
        File(mapFiles, "1000-2000").writeBytes("fixture".toByteArray())
        File(process, "maps").writeText(
        ...
        assertNull(ProcControlModuleFingerprintProvider.sha256(PID, DELETED_PATH, 0x1000, procControl))
    }
