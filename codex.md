...
> **当前阶段：** 产品 UI、证书/shared UID/Binder 控制链、typed 读写、native probe 和防闪主路径已闭合；控制命名空间重构已完成，协议版本为 v11。
> **执行原则：** 先保持产品、权限、悬浮窗和已闭合功能可用，再按证据逐项开放目标进程功能。任何身份、版本、映射或字段证据不完整的功能均保持 fail-closed。

本文是当前代码的唯一迁移执行基线。控制实现统一位于 `me.dartcv.minix.control`，AIDL descriptor 为 `me.dartcv.minix.control.IControlBridge`，Service 为普通非导出 `:control` 进程。控制链由证书身份、`android:sharedUserId`、安装后同 Linux UID、Binder caller UID、PID/startTime 和 maps/SO 指纹门禁组成。
...
| IPC | `android:exported="false"` 的 `ControlService`，运行于 `:control`，通过 `IControlBridge` AIDL/Binder 调用 |
| 协议 | v11；descriptor/package 迁移后旧客户端在连接阶段失配 |
| UID 门禁 | PackageManager 分配的安装 UID、服务 UID、Binder caller UID 与目标 effective UID 逐层核对 |
| native 边界 | 仅在四元进程身份、完整 SO SHA 和 scoped maps 通过后执行 typed probe/读写 |
| 重构状态 | `control` 包、类名、测试、AIDL 和 JNI 导出已统一，旧命名空间与别名已移除 |

### 0.1 重构验证记录（2026-08-18）

本轮从 clean 状态执行了单元测试、lint、CMake/native、Debug 和 Release 构建：

```text
BUILD SUCCESSFUL
36 suites / 270 tests / 0 failures / 0 errors / 0 skipped
lint: 0 errors / 15 warnings
APK signing: v2 only / 1 signer / certificate SHA-256 A40DA80A59D170CAA950CF15C18C454D47A39B26989D8B640ECD745BA71BF5DC
generated AIDL: debug/out/me/dartcv/minix/control/IControlBridge.java
                 release/out/me/dartcv/minix/control/IControlBridge.java
```
...
| 产物 | 大小 | SHA-256 |
|---|---:|---|
| `app/build/outputs/apk/debug/app-debug.apk` | 58,948,575 | `62A0667BF9F553C1E5F035EA9453734D6515314ED789653C0504B3D4B37A18FA` |
| `app/build/outputs/apk/release/app-release.apk` | 42,829,972 | `5F4F2690F2047228D10C6E4F6AED66D1E4CB3C88650A53020A028FC7D361CC28` |

两个 APK 的 DEX、Manifest 和 native library 均未包含旧包路径、旧 Binder descriptor 或旧 JNI 前缀；native 导出仅保留 `Java_me_dartcv_minix_control_nativeadapter_*`。静态契约脚本 `verify_headspin_native_batch_contract.ps1` 与 `verify_headspin_public_surface.ps1` 均返回 `PASS`。
...
- 当前控制链固定使用证书、shared UID、普通 Binder Service 与受身份门禁保护的 native probe。
...
| 本地 Binder 控制桥 | 已完成 | 普通非导出 Service、`:control` 进程、显式 `bindService()`、协议 v11 |
...
- `ControlService` 继承普通 `android.app.Service`。
...
- AIDL 协议版本为 11；客户端校验协议版本、服务 PID、服务 ABI 和服务 UID。SearchID 命令额外回传请求 ID、typed 状态、可选槽位、失败原因和进程 startTime，客户端拒绝形状不一致的响应。
...
构建配置为 `minSdk = 28`。AGP 9.1.1 当前 clean 产物由外部固定证书签名，`apksigner 37.0.0` 验证结果为 v2-only、单 signer；签名方案封装不参与 shared UID 的身份判断。

必须区分两件事：

- **共享 Linux UID 的身份条件**是安装包使用兼容的签名证书身份，并声明相同的 `android:sharedUserId`；最终是否分配同一 UID 由设备 PackageManager 决定。
- **APK Signature Scheme v2/v3**只是 APK 的签名封装与校验方案。签名方案本身不会创建 shared UID，也不会替代证书身份和 `sharedUserId`。

因此发布时不能只看“APK 签名方案已验证”。必须同时核对 MINIX 与目标包的活动 signer、目标包 `sharedUserId` 和安装后 Linux UID。
...
- `app/src/main/java/me/dartcv/minix/control/ControlGameAppArtifact1582.kt`
...
| SearchID | 静态闭合 | exact-SHA BSS anchor；`anchorOffset=0x18760`；40 槽固定扫描；AIDL v11 与控制页 int32 输入 | UI 与 typed 结果已接通；真机命中结果待有效房间 ID |
...
5. AIDL v11 同时返回请求 ID、槽位、失败原因和 startTime；客户端校验布尔命令结果与 typed payload 一致性。Controller 只保存属于原 PID/startTime 的结果，目标变化、关闭或断连时清除。
...
5. 每个成功 iteration 后等待 `22 ms`，再开始下一轮；worker 由 `ControlService` 持有。
...
- `ControlController.disconnect()`、渠道切换和目标关闭均由协程互斥锁串行；回滚未完成时保留 Binder 连接和目标会话，不提前销毁恢复所需上下文。

状态通过协议 v11 暴露，主要状态包括 `WAITING_FOR_TARGET`、`RUNNING`、`STOPPING`、`STOPPED`、`TARGET_CHANGED`、`PROFILE_MISMATCH`、`READ_FAILED`、`WRITE_FAILED`、`VERIFY_FAILED`、`ROLLBACK_FAILED` 和 `BACKEND_UNAVAILABLE`。UI 只有在 `requestedEnabled && workerRunning` 时显示防闪已启用。
...
- [x] 接入 SearchID、LIFE_STATE、KILL_COUNT 和 GetDataLong(1) typed 读取；SearchID 已接通 AIDL v11、Controller 生命周期和控制页输入/结果，LIFE_STATE 已用当前 exact-SHA `AttrHPComponent` 配方替换旧 masked-H 草案。
...
已完成真机设备、ABI、SELinux、配套包 signer/shared UID/安装 UID，以及 ANTI_FLASH、FLIGHT、PLAYER_TELEPORT、FAKE_FLIGHT 写链、KILL_COUNT 和 GetDataLong(1) 的主路径验收。当前 clean 重构构建的 Debug/Release 产物、测试和 lint 结果见 §0.1；历史真机证据仍绑定各自记录的 APK 哈希。LIFE_STATE 当前版配方另有 13/13 静态检查，HITBOX evidence verifier 为 39/39；本轮构建时 ADB 设备列表为空，故新 APK 覆盖安装、LIFE_STATE 实读和 SearchID UI Automator 取证排在设备重新连接后的首位。后续只推进真机 SearchID/LIFE_STATE 结果、故障注入、人工效果验收和仍缺唯一地址证据的功能：
...
2026-08-18 对当前代码执行 clean 全矩阵构建：

```text
test result: BUILD SUCCESSFUL; 36 suites / 270 tests / 0 failures / 0 errors / 0 skipped
build result: BUILD SUCCESSFUL; clean + assembleDebug + assembleRelease passed
...
- `ControlService exported=false` 且 `process=:control`
...
SetheadSpeed 的 JNI setter 已闭合为 `libClient+0x58918c` 唯一写 `libClient+0x971e04`，宽度 4B，保存值为 `int32(input)*10` 微秒；唯一 consumer worker 为 `0x589698`，每轮通过 H 链取得 `p3+0x54/+0x58` 两个 float 轴，计算 `fmod(angle+1.0f,361.0)` 后写回，再按 delay 休眠。正式 selector 域扫描已闭合为 selector `17`，生命周期审计同时确认原 detached worker 的停用路径不适合作为运行时入口。MINIX 现在保留 `HeadSpin.kt` 的单 worker gated supervisor 和 `ControlHeadSpinNativeBackend.kt` 的 maps-aware 批处理适配，使用 exact-SHA 代码字 `0xb9005909` 与恢复字 `0x72a80bea`、会话轴快照和受保护回滚；adapter conversion fixtures 已覆盖 unknown after、observed verify failure 和 partial pair。`headspin_ui_contract.json` 记录 UI 10..955、raw=1000-ui 以及 selector 17/18 分流，`tools/verify_headspin_native_batch_contract.ps1` 对 native loop 的 8/8 completed-count 不变量做静态检查。`work/headspeed-recovery-20260817/headspin_minix_verification.json` 记录 36 suites/271 tests（HeadSpin 12 + adapter 7）、lint、native contract 8/8、arm64 native build 与 Debug/Release assemble 全部通过；target-side native/runtime 仍 pending，故 `production_ready=false`，ControlFeature/AIDL/UI 列表暂不改动。 正式专项报告为 `docs/2026-08-18_headspin-gated-implementation-report.md`。 Debug `app/build/outputs/apk/debug/app-debug.apk` SHA-256 `290F1272CE6E90CCABF2412983861D422383ADC6399B55641DB88296B1B7662B`、Release SHA-256 `9B7006927CD45236486E52ECF3EEF3BF4522860BEA878A494DA337BE2F20C768`。
...
| Binder 服务 | `app/src/main/java/me/dartcv/minix/control/ControlService.kt` |
| Binder 客户端 | `app/src/main/java/me/dartcv/minix/control/ControlBridgeClient.kt` |
| 防闪档案、resolver、监督器与状态机 | `app/src/main/java/me/dartcv/minix/control/ControlAntiFlash.kt` |
| 防闪 JNI 后端与 deleted 映射散列 | `app/src/main/java/me/dartcv/minix/control/ControlNativeProbe.kt` |
| native adapter 与 wire decoder | `app/src/main/java/me/dartcv/minix/control/nativeadapter/TargetNativeProbe.kt` |
| 控制生命周期与回滚保留 | `app/src/main/java/me/dartcv/minix/control/ControlController.kt` |
| 控制传输策略元数据 | `app/src/main/java/me/dartcv/minix/transport/TransportPolicy.kt` |
| 包安装身份预检 | `app/src/main/java/me/dartcv/minix/control/TargetPackageIdentityVerifier.kt` |
| 四元目标会话 | `app/src/main/java/me/dartcv/minix/control/ControlTargetSession.kt` |
| GameApp 静态身份 | `app/src/main/java/me/dartcv/minix/control/ControlGameAppArtifact1582.kt` |
| head_spin 内部 supervisor 与 native adapter | `app/src/main/java/me/dartcv/minix/control/HeadSpin.kt`、`app/src/main/java/me/dartcv/minix/control/ControlHeadSpinNativeBackend.kt` |
| head_spin fixture/native contract checks | `app/src/test/java/me/dartcv/minix/control/HeadSpinTest.kt`、`app/src/test/java/me/dartcv/minix/control/HeadSpinNativeBackendTest.kt`、`tools/verify_headspin_native_batch_contract.ps1` |
| maps/BSS anchor | `app/src/main/java/me/dartcv/minix/control/ProcControlSearchIdAnchorResolver.kt` |
| 只读字段档案 | `app/src/main/java/me/dartcv/minix/control/ControlReadOnlyFieldProfiles.kt` |
| typed 写入档案 | `app/src/main/java/me/dartcv/minix/control/ControlInjection.kt` |
...
`目标包 sharedUserId 不匹配: <空>`；授予应用权限不会改变 PackageManager 的安装身份。
